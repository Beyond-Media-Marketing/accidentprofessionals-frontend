...
const postsQuery = qs.stringify(
  {
    ...
    populate: { coverImage: true, category: { fields: ['name', 'slug'], populate: { parent: { fields: ['name', 'slug'] } } } },
    pagination: { pageSize: 100 },
  },
  { encodeValuesOnly: true },
)
const catsQuery = qs.stringify(
  { sort: ['order:asc', 'name:asc'], fields: ['name', 'slug'], filters: { parent: { id: { $null: true } } }, pagination: { pageSize: 100 } },
  { encodeValuesOnly: true },
)
...
const { data } = await useAsyncData('blogs-index', async () => {
  const [postsRes, catsRes] = await Promise.all([
    ...
    $fetch<any>(`${strapiUrl}/api/blog-categories?${catsQuery}`).catch(() => null),
  ])
...
const rest = computed(() => posts.value.filter((p) => p.slug !== featured.value?.slug))
  ...
  <div>
    ...
    <section class="bg-dark text-on-dark">
      <div class="site-container py-16 text-center 3xl:py-20">
        <span class="mb-5 inline-block rounded-pill border border-accent/60 px-4 py-1.5 font-primary text-xs font-semibold uppercase tracking-wider text-accent">Insights &amp; Resources</span>
        <h1 class="font-secondary text-[clamp(2.2rem,5vw,3.5rem)] font-bold leading-[1.08]">
          The Accident Professionals <span class="text-accent">Blog</span>
        </h1>
        <p class="mx-auto mt-5 max-w-[640px] font-primary text-[15px] leading-[1.7] text-white/70 3xl:text-base">
          Straight-talking guides on what to do after an accident, how claims really work, and how to protect your rights in Georgia.
        </p>
      </div>
    </section>
    ...
    <section class="section bg-cream">
      <div class="site-container">
        <!-- Category filter -->
        <div v-if="categories.length" class="mb-10 flex flex-wrap justify-center gap-2.5 3xl:mb-14">
          <span class="rounded-pill bg-dark px-5 py-2.5 font-primary text-sm font-semibold text-white">All Posts</span>
          <NuxtLink
        ...
        <template v-if="posts.length">
          <!-- Featured -->
          <BlogCard v-if="featured" :post="featured" featured class="mb-10 3xl:mb-14" />
