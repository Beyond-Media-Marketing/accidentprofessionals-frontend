...
const date = computed(() =>
  props.post?.publishedDate
    ? new Date(props.post.publishedDate).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      })
...
<template>
  ...
  >
    ...
    <div class="relative overflow-hidden" :class="featured ? 'sm:w-[54%]' : ''">
      <div
        :class="
          featured ? 'aspect-[16/10] h-full sm:aspect-auto' : 'aspect-[16/10]'
        "
      >
        <img
          :src="cover"
    ...
    <!-- Body -->
    <div
      class="flex flex-1 flex-col p-6 3xl:p-7"
      :class="featured ? 'sm:justify-center sm:p-8 3xl:p-10' : ''"
    >
      <p
        v-if="date"
        class="mb-3 font-primary text-xs font-medium uppercase tracking-wide text-muted"
      >
        {{ date }}
      </p>
      <h3
      ...
      <span
        class="mt-5 inline-flex items-center gap-1.5 font-primary text-sm font-semibold text-accent"
      >
        Read article
        <svg
